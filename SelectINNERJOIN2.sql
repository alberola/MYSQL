...
select empleados.nombre, YEAR(curdate())-YEAR(`fnacimiento`) as 'Edad', sum(ingreso-descuento) as 'Sueldo Neto'
from (empleados inner join just_nominas on empleados.codigo = just_nominas.cod_emp) where empleados.fnacimiento in 
(select min(fnacimiento) from empleados) or empleados.fnacimiento in (select max(fnacimiento) from empleados) group by just_nominas.cod_emp;
